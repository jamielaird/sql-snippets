--Create the field [Master Enquiry Source] and classify each record as Email, Phone, Webform, Fairs & Exhibitions or Irregular Lists
  CASE
    ...
    ST.first_source_tag LIKE '%netnatives%' THEN 'Netnatives'
    WHEN ST.first_source_tag LIKE '%[_]fb[_]%' OR
    ST.first_source_tag LIKE '%facebook%' THEN 'Facebook'
    WHEN ST.first_source_tag LIKE '%[_]cx[_]%' OR
    ST.first_source_tag LIKE '%cappex%' THEN 'Cappex'
    WHEN ST.first_source_tag LIKE '%[_]ms[_]%' OR
    ST.first_source_tag LIKE '%masterstudies%' THEN 'Masterstudies'
    WHEN ST.first_source_tag LIKE '%[_]hc[_]%' OR
    ST.first_source_tag LIKE '%hotcourses%' THEN 'Hotcourses'
    WHEN ST.first_source_tag LIKE '%[_]gecko[_]%' OR
    ST.first_source_tag LIKE '%gecko%' OR
    ST.first_source_tag LIKE '%gekko%' THEN 'Gecko'
    WHEN ST.first_source_tag LIKE '%[_]11ten[_]%' OR
    ST.first_source_tag LIKE '%11ten%' THEN '11Ten'
    ...
    WHEN CONVERT(date, ST.first_source_tag_date_created) = CONVERT(date, S.date_created) AND
    ST.first_source_tag LIKE '%emt%' THEN 'Enquiry forms'
    WHEN CONVERT(date, ST.first_source_tag_date_created) = CONVERT(date, S.date_created) AND
    ...
    WHEN ST.first_channel_mod = 'Prospect List' AND
    ST.first_source_tag LIKE '%_ff_%' THEN 'Fairs & Exhibitions'
    WHEN ST.first_channel_mod = 'Prospect List' AND
    ST.first_source_tag LIKE '%_bc_%' THEN 'Irregular Lists'
    WHEN ST.first_channel_mod = 'Prospect List' AND
